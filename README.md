...
Поля: 
 - пользователь
 - оплаченный курс и отдельно оплаченный урок должны быть ссылками на соответствующие модели.
...
Модель подписки должна содержать следующие поля: «пользователь» (FK на модель пользователя), «курс» (FK на модель курса).
...
### Задание 1
одключить и настроить вывод документации для проекта. Убедиться, что каждый из реализованных эндпоинтов описан в документации верно, при необходимости описать вручную.
...
Чтобы реализовать рассылку, вызывайте специальную задачу по отправке письма в коде контроллера. 
...
2. Откройте файл .env и заполните все переменные окружения.

### Запуск
Соберите образы и запустите контейнеры одной командой:

#### docker compose up --build

#### Примечание: 
при первом запуске инициализация базы данных занимает 30–40 секунд. 
Не прерывайте процесс, пока не увидите сообщение Watching for file changes with StatReloader в логах контейнера web.

### Как понять, что приложение работает
 - Контейнер db имеет статус Healthy.
 - В логах контейнера web есть строка Watching for file changes with StatReloader.
 - Откройте в браузере http://127.0.0.1:8000 — должна загрузиться страница приложения.
   Для проверки работоспособности используйте один из основных эндпоинтов

### Остановка контейнеров
Чтобы корректно остановить и удалить контейнеры:

#### docker compose down

### Пересборка после изменений в Dockerfile или зависимостях
Если вы изменили Dockerfile или добавили пакеты в pyproject.toml/poetry.lock, используйте:
...
1. Запускать тесты проекта автоматически при каждом push в репозиторий.
2. Автоматически деплоить проект на удаленный сервер после успешного прохождения тестов. 
