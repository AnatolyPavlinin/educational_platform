...
class LmsApiTests(APITestCase):
    ...
    def test_lesson_creation_by_owner(self):
        ...
        print("DEBUG CREATE:", response.status_code,
    ...
    def test_permissions_cross_access(self):
        ...
        resp_patch = client_other.patch(self.detail_url, {'title': 'Hack'}, format='json')
    ...
    def test_subscribe_toggle(self):
        ...
        self.assertEqual(response.json()['message'], 'Вы успешно подписались на обновления курса.')

        # 2. Отписываемся (удаляем запись)
        response = client.post(self.subscribe_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(CourseSubscription.objects.filter(user=self.user, course=self.course).exists())
        self.assertEqual(response.json()['message'], 'Вы успешно отписались от обновлений курса.')
